...
export default function Testimonials() {
  ...
  return (
    <section className="overflow-hidden" style={{ background: '#fff', paddingTop: '80px' }} ref={ref}>
      ...
      {/* ── Cards ── */}
      <div className="flex flex-col lg:flex-row items-stretch gap-0 px-5 md:px-10 lg:px-[52px]">
          ...
          return (
            <div
              ...
              style={{
                ...
                boxShadow: isFeatured ? '0 24px 60px rgba(10,37,88,0.22)' : 'none',
            ...
            >
              ...
              <div className="font-black leading-none mb-5 select-none"
                style={{ fontSize: '80px', lineHeight: 0.8, color: isFeatured ? 'rgba(249,115,22,0.25)' : 'rgba(10,37,88,0.08)', fontFamily: 'Georgia, serif' }}>
                "
              </div>
              ...
              <p className="leading-relaxed flex-1 mb-6"
                style={{ fontSize: '15px', color: isFeatured ? 'rgba(255,255,255,0.95)' : '#374151', lineHeight: '1.75' }}>
                "{t.quote}"
              </p>

              <div className="mb-5 h-px" style={{ background: isFeatured ? 'rgba(255,255,255,0.1)' : '#e8eef8' }} />

              <div className="flex items-center gap-4">
                <div className="shrink-0 w-11 h-11 rounded-full flex items-center justify-center font-black text-base"
                  style={{ background: isFeatured ? '#f97316' : '#0a2558', color: 'white', boxShadow: isFeatured ? '0 6px 20px rgba(249,115,22,0.4)' : '0 4px 14px rgba(10,37,88,0.2)' }}>
                  {t.name[0]}
                </div>
                <div>
                  <p className="font-black text-sm" style={{ color: isFeatured ? 'white' : '#0a2558' }}>{t.name}</p>
                  <p className="text-xs" style={{ color: isFeatured ? 'rgba(255,255,255,0.6)' : '#9ca3af' }}>{t.role} · {t.location}</p>
                </div>
                <div className="ml-auto flex items-center gap-1 rounded-full px-3 py-1" style={{ background: isFeatured ? 'rgba(255,255,255,0.1)' : '#f0fdf4' }}>
      ...
      {/* ── Stats strip ── */}
      <div className={`grid grid-cols-2 md:grid-cols-4 mt-12 anim-fade-up ${inView ? 'anim-in' : ''}`}
        ...
        {stats.map((s, i) => (
          <div key={s.label} className="flex flex-col items-center justify-center py-8"
            ...
            <p className="font-black text-white leading-none mb-1" style={{ fontSize: 'clamp(24px, 3vw, 34px)' }}>{s.num}</p>
