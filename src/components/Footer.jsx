...
export default function Footer() {
  ...
  return (
    <footer style={{ background: '#071a3e' }} ref={ref}>
      ...
      {/* ── TOP CTA BAND ── */}
      <div className={`relative overflow-hidden anim-fade-up ${inView ? 'anim-in' : ''}`} style={{ background: 'linear-gradient(135deg, #0d3070 0%, #0a2558 100%)', padding: '52px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        {/* Decorative circle */}
        ...
        <div className="absolute rounded-full" style={{ width: '160px', height: '160px', background: 'rgba(249,115,22,0.05)', left: '40%', bottom: '-60px' }} />

        <div className="relative z-10 flex items-center justify-between">
          <div>
            <p className="text-orange-400 font-bold text-xs tracking-widest uppercase mb-2">Ready to Travel?</p>
            <h3 className="font-black text-white leading-tight" style={{ fontSize: '32px' }}>
              Book Your Ride Today —<br />
              <span style={{ color: '#f97316' }}>Safe, Fast & Reliable.</span>
            </h3>
          </div>
          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-3 rounded-2xl px-5 py-4" style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)' }}>
              <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ background: 'rgba(249,115,22,0.2)' }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2" className="w-5 h-5">
                  <path d="M6.62 10.79a15.05 15.05 0 0 0 6.59 6.59l2.2-2.2a1 1 0 0 1 1.01-.24c1.12.45 2.33.7 3.58.7a1 1 0 0 1 1 1V20a1 1 0 0 1-1 1A17 17 0 0 1 3 4a1 1 0 0 1 1-1h3.5a1 1 0 0 1 1 1c0 1.25.25 2.46.7 3.57a1 1 0 0 1-.23 1.01l-2.35 2.21z" />
                </svg>
              </div>
              <div>
                ...
                <p className="text-white font-black" style={{ fontSize: '18px' }}>(123) 456-7890</p>
              </div>
            </div>
            <button
              className="font-black text-white rounded-2xl tracking-widest"
              style={{
                padding: '18px 36px',
                fontSize: '13px',
      ...
      {/* ── MAIN FOOTER ── */}
      <div style={{ padding: '64px 52px 48px' }}>
        <div className="grid gap-12" style={{ gridTemplateColumns: '1.4fr 1fr 1fr 1.2fr' }}>

          {/* Col 1: Logo + about */}
          <div className={`anim-fade-up ${inView ? 'anim-in' : ''}`} style={{ transitionDelay: '0.15s' }}>
            {/* Logo container */}
            <div className="inline-flex items-center justify-center rounded-2xl mb-5"
              ...
              <img src={logo} alt="Delta Care Transport" style={{ height: '72px', width: 'auto' }} />
            </div>
            ...
            <p className="text-blue-100 leading-relaxed mb-6" style={{ fontSize: '14px' }}>
              ...
              and families with care across Atlanta and beyond.
            </p>
          ...
          <div className={`anim-fade-up ${inView ? 'anim-in' : ''}`} style={{ transitionDelay: '0.25s' }}>
            ...
            <ul className="flex flex-col gap-3">
              {quickLinks.map((l) => (
                <li key={l}>
                  <a href="#" className="group flex items-center gap-3 transition-colors"
                    style={{ color: '#93c5fd' }}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2.5" className="w-3 h-3 shrink-0 transition-transform group-hover:translate-x-1">
          ...
          <div className={`anim-fade-up ${inView ? 'anim-in' : ''}`} style={{ transitionDelay: '0.45s' }}>
            ...
            <div className="flex flex-col gap-4">
              {contactItems.map((c) => (
                <div key={c.label} className="flex items-center gap-4 rounded-xl p-3"
      ...
      {/* ── BOTTOM BAR ── */}
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '20px 52px' }}>
        <div className="flex items-center justify-between">
          <p className="text-blue-200 text-sm">
            © 2024 <span className="text-white font-semibold">Delta Care Transport</span>. All rights reserved.
          </p>
          <div className="flex items-center gap-6">
            {['Privacy Policy', 'Terms of Service', 'Non-Medical Transportation'].map((item, i) => (
              <span key={item} className="flex items-center gap-6">
                {i > 0 && <span className="text-blue-800">·</span>}
                <a href="#" className="text-blue-200 text-sm hover:text-orange-400 transition-colors">{item}</a>
              </span>
            ))}
          </div>
        </div>
